...
if __name__ == "__main__":
    default = main.readstate("cookie.txt")
    if default == "":
    ...
    if a != None:
        main.writestate(a, "cookie.txt")
    else:
        exit()
    default = main.readstate("website.txt")
    if default == "":
        a = gui("Please enter your SEQTA URL (with https://).", "New user detected!", default)
    else:
        a = gui("Please enter your SEQTA URL (with https://).", "Change SEQTA URL", default)
    if a != None:
        main.writestate(a, "website.txt")
    else:
        exit()
    a = update.updater()
    if a.content == b"{\"status\":\"401\"}" or "":
